...
type Config struct {
	...
	SilentATK      bool            `json:"silentATK"`
	Algorithm      string          `json:"algo"`
	OutputLog      bool            `json:"outputLog"`
	Seed           int64           `json:"seed"`
	TrueDelay      int             `json:"trueDelay"`
	BigDelta       int             `json:"bigDelta"`
	DelayRange     int             `json:"delayRange"`
...
func (c Config) GetTrueDelay() time.Duration {
	rand.Seed(time.Now().UnixNano())
	min := c.TrueDelay - c.DelayRange
	max := c.TrueDelay + c.DelayRange
	delay := rand.Intn(max-min+1) + min
	return time.Duration(delay) * time.Millisecond
}

func (c Config) GetBigDelta() time.Duration {
	return time.Duration(c.BigDelta) * time.Millisecond
}
